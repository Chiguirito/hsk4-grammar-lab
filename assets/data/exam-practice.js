...
Same subject in both clauses → subject sits BEFORE the connector, once: <span class="zh"><b class="hl">他</b>不但会开车，而且开得很好</span>. Different subjects → each clause keeps its own, after the connector: <span class="zh">不但<b class="hl">他</b>会，<b class="hl">他妹妹</b>也会</span>. Ordering items exploit this constantly.</div>`
    ...
    /* ------------------------------------------------ 2 */
    {
      ...
      items: [
        ...
        { q: '<span class="zh">A 那就好好儿准备明年的比赛吧<br>B 既然你已经决定再试一次<br>C 也别再为这次的失败难过了</span>', choices: ["A → B → C", "B → A → C", "C → B → A", "B → C → A"], a: 1, expl: "既然 (B) opens and 那就 (A) answers it — the pair 既然…那就 is glued. The 也 in C hooks it onto the advice before it, so C can't open — it closes. — 'Since you've decided to try again, prepare well for next year's competition, and don't feel bad about this failure any more either.'" },
        ...
        { q: '<span class="zh">A 汉语的声调一开始很难掌握<br>B 但是只要每天坚持练习<br>C 就一定能说得越来越标准</span>', choices: ["B → C → A", "A → C → B", "A → B → C", "C → B → A"], a: 2, expl: "A states the difficulty; 但是 (B) turns; 只要…就 chains B to C. Two paired clues in one item — 就 confirms C can't stand first. — 'Tones are hard at first, but if you practice daily, you'll speak more and more accurately.' (掌握 zhǎngwò = to master, HSK5.)" },
        { q: '<span class="zh">A 他把收件人的地址写错了<br>B 所以包裹又被寄回来了<br>C 第一次寄包裹的时候</span>', choices: ["A → C → B", "C → A → B", "C → B → A", "A → B → C"], a: 1, expl: "Time frame first (C: …的时候), cause second (A: the 把-machine reports what he did to the address), 所以-result last (B: the 被-machine reports what happened to the parcel). — 'The first time he mailed a parcel, he wrote the address wrong, so it got sent back.' (收件人 shōujiànrén = the recipient.)" },
...
The exam pre-glues characters into chunks of 2–4 — and gluing is information! A chunk like <span class="zh">学了</span> (verb+了) wants a duration right behind it; a chunk like <span class="zh">是去年九月</span> screams for a <span class="zh">的</span> tile somewhere at the end (是…的). Read the glue, not just the words.</div>`
    ...
    /* ------------------------------------------------ 4 */
    {
      ...
      items: [
        ...
        { tiles: ["这个字", "太难了", "连老师", "都不认识"], py: "Zhège zì tài nán le, lián lǎoshī dōu bú rènshi.", en: "This character is so hard even the teacher doesn't know it.", hint: "连 X 都 — the extreme case, with 都 glued after it.", alt: [["这个字", "连老师", "都不认识", "太难了"], ["连老师", "都不认识", "这个字", "太难了"]] },
        ...
        { tiles: ["他昨晚", "只睡了", "五个小时的", "觉"], py: "Tā zuówǎn zhǐ shuì le wǔ ge xiǎoshí de jiào.", en: "He only got five hours of sleep last night.", hint: "睡觉 is separable — the duration (＋的) wedges in between 睡 and 觉." },
...
Every wrong option in the quiz below is a real learner error — the same ones from the error clinics across this site. If an option feels comfortable, that's exactly why it's there. Trust the checks, not the comfort.</div>`
    ...
    /* ------------------------------------------------ 7 */
    {
      ...
      items: [
        ...
        { q: '<span class="zh">他最近忙___连饭都忘了吃。</span>', choices: ['<span class="zh">的</span>', '<span class="zh">地</span>', '<span class="zh">得</span>', '<span class="zh">了</span>'], a: 2, expl: "连饭都忘了吃 states the RESULT of how busy he is → degree complement → 得 (忙得连…都…). 地 marks the manner of a deliberate action (认真地工作) — forgetting to eat isn't something you do on purpose. (see: the three de)" },
        ...
        { q: '<span class="zh">教室里有___学生。</span>', choices: ['<span class="zh">三十来个</span>', '<span class="zh">三十个来</span>', '<span class="zh">来三十个</span>', '<span class="zh">三来十个</span>'], a: 0, expl: "来 slots between a round ten and the measure word: 三十 + 来 + 个. Every other position is scrambled. (see: numbers & approximation)" },
