...
Listening plants minimal pairs (<span class="zh">开着</span> vs <span class="zh">开了</span>), reading uses existential 着-sentences (<span class="zh">墙上挂着…</span>) in every description, and the picture-writing task practically begs for <span class="zh">穿着/拿着/坐着</span>.</div>`
    ...
    /* ------------------------------------------------ 3 */
    {
      ...
      items: [
        ...
        { tag: "V着V着", cn: "孩子看**着**看**着**电视就睡着了。", py: "Háizi kànzhe kànzhe diànshì jiù shuìzháo le.", en: "Watching TV, the kid gradually fell asleep.", note: "Same character, two readings: 看着 kànzhe (durative) but 睡着 shuìzháo — 着 zháo is a result complement 'succeed in falling asleep'." },
...
The classic listening trap: 门开着 (it's open) vs 门开了 (it just opened) vs 正在开门 (someone is opening it). Three sounds, three pictures — the question hinges on which picture matches the dialogue.</div>`
    ...
    /* ------------------------------------------------ 5 */
    {
      ...
      items: [
        ...
        { q: '<span class="zh">孩子看___看___电视就睡着了。</span>', choices: ['<span class="zh">着…着</span>', '<span class="zh">了…了</span>', '<span class="zh">过…过</span>', '<span class="zh">完…完</span>'], a: 0, expl: "V着V着 = 'and as the action went on…' — the gradual-slide frame. The final 睡着 (shuìzháo) is a different 着 altogether: a result complement." },
...
German <i>mal</i> is VV in one word: <i>„Schau <b>mal</b>“</i> = <span class="zh">看看</span>. Spanish softens the same way with a diminutive or “un poco”: <i>“espera un poquito”</i> = <span class="zh">等等</span>. The three de's page (的地得) already showed you AABB + 地 — this is where those chunks come from.</div>`
    ...
    /* ------------------------------------------------ 9 */
    {
      ...
      items: [
        ...
        { wrong: "一张地图挂着在墙上。", right: "地图挂在墙上。", py: "Dìtú guà zài qiáng shàng.", en: "The map hangs on the wall.", expl: "V着 can't take 在 + place behind it — the one bug is the stacked 着. Either the location result (挂在墙上) or the existential frame (墙上挂着一张地图) — never 挂着在." }
      ]
    },
