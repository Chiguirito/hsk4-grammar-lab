...
**P1 (a weekend):** add the Appendix `alt` orders · fix the 4 clinic items and 8 softened claims (§2.3-2.4) · polish nits (§2.5) · `lang="zh-Hans"` + button tiles (§3.2) · shell generator for titles/descriptions/OG/favicon (§3.3) · run validate.js in CI (§3.5) · fonts preconnect + toggle-flash fix (§3.4). — ✅ **Done** (49 alt orders added with multiset verification; shells now generated by `scripts/build-shells.js`, checked in CI alongside `validate.js`; all verified in-browser).

**P2 (ongoing):** choice shuffling · "review my misses" mode · print stylesheet · dark mode · PWA · the 着/reduplication/是否 topic. — ✅ **Done** (all six shipped, plus the smaller P2 items: contextual builder hints, tap-again-for-slow TTS, speakable table sentences, honest hero stats, and progress tooltips; 18 explanations that referenced option letters were rewritten to survive choice shuffling). Not implemented: global 1-4 keyboard shortcuts for quizzes — with many quiz sections per page the mapping is ambiguous, and every control is already keyboard-reachable via Tab/Enter.
