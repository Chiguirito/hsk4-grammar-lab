...
## Vocabulary coverage (side effect, not a goal)

The site tracks how much of the HSK-4 target vocabulary — (old HSK 4 ∪ new HSK 4)
minus the HSK 1–3 lists the learner knows — appears in the content. Words from that
set are automatically underlined in the browser with a tap-for-translation popover
(`assets/data/vocab.js`, regenerated by `scripts/build-vocab.js`; needs network).

- **When writing or editing sentences, prefer words from the missing-word list**:
  `node scripts/vocab-coverage.js` prints coverage stats and the most common
  still-missing target words. `--topic <id>` inspects one topic; `--json` for tooling.
- **Never bend a sentence to smuggle a word in.** An unnatural example teaches the
  grammar worse — coverage is a side effect of good examples, not a quota.
- Grammar-jargon compounds (句型, 补语…) that the underliner would mis-segment go
  into `EXTRA_DICT` in `scripts/build-vocab.js`.
